...
const Remove = ({ formToken, stableToken, token0, token1, route0, route1 }) => {
	...
	const [selectedValue, setSelectedValue] = useState({
		tokenFrom: { name: options[0].name, icon: options[0].icon, width: options[0].width, height: options[0].height },
		tokenTo: { name: options[1].name, icon: options[1].icon, width: options[1].width, height: options[1].height },
	});
	...
	useEffect(() => {
		if ([1, 4].includes(chainId)) {
			...
			setSelectedValue({
				tokenFrom: { name: options[0].name, icon: options[0].icon, width: options[0].width, height: options[0].height },
				tokenTo: { name: options[2].name, icon: options[2].icon, width: options[2].width, height: options[2].height },
			});
		}
	}, [chainId]);
		...
		const getLiquidity = async () => {
			if (uniswapPair && lpToken && account && removeValue) {
				...
				const userBalanceToRemove = new TokenAmount(
					pair.liquidityToken,
					convertToUnit(removeValue?.isNaN() ? 0 : removeValue?.toFixed(18), 18)
				);
	...
	const onSelectRemoveValue = (amount) => {
		...
		setRemoveValue(new BigNumber(userBalance).multipliedBy(new BigNumber(amount/100)));
	};
	...
	const handleApproval = async (event) => {
		...
		if (!currentAllowance) {
			await handleLPTokenApproval({ successMessage: 'You can now remove LP tokens.', infoMessage: 'Enabling..' });
		}
	};
		...
		<div className='swapper'>
			...
			<div className='d-flex flex-direction-column flex-direction-row@phone align-items-center space-h--mobile'>
				<Button
					...
					onClick={!currentAllowance ? handleApproval : invokeRemoveLiquidity}
					disabled={
						!removeValue ||
						swapError ||
						!account ||
			...
			<div className='liquidity-tab__remove__summary'>
				...
				<div>
					<p className='font-size-12'>1 {selectedValue.tokenFrom.name} = {exchangeRate().fromPrice} {selectedValue.tokenTo.name}</p>
					<p className='font-size-12'>1 {selectedValue.tokenTo.name} = {exchangeRate().toPrice} {selectedValue.tokenFrom.name}</p>
				</div>
			</div>
		</div>
	);
