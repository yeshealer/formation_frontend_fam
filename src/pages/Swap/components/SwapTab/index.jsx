...
const SwapTab = ({
  ...
  const invokeSwap = async () => {
    ...
    try {
      ...
      notify("error", "Tolerance slippage was too high", "Please try again later.");
    }
  };
  ...
  const renderButtonTitle = () => {
    ...
    if (!stableTokenAllowance && (selectedValue?.tokenFrom?.name === 'USDT' || selectedValue?.tokenFrom?.name === 'BUSD')) {
  ...
  const handleButtonAction = () => {
		...
		if (!stableTokenAllowance && (selectedValue?.tokenFrom?.name === 'USDT' || selectedValue?.tokenFrom?.name === 'BUSD')) {
			return handleStableTokenApproval({ successMessage: `${selectedValue?.tokenFrom?.name} is enabled!` });
		}
  ...
  return (
    <>
      <div className="swap-tab">
        <Header />
        {/* <InfoReminder infoTextMain={'Swap / Liquidity is temporary disabled during migration update.'} /> */}
