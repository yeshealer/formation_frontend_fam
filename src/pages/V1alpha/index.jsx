...
const V1alpha = () => {
  ...
  const startAction = (amount) => {
    ...
    (async () => {
      if (account && chainId && library) {
        ...
        try {
          await v1alphaBalanceWeb3.methods.deposit(new BigNumber(amount).multipliedBy(10 ** 18)).call();
        } catch (err) {
          console.log(err);
          // console.log('err');
          console.log('error');
  ...
  useEffect(() => {
		console.log(stableTokenBalance);
		console.log('stableTokenBalance');
	}, [v1alphaTokenBalance]);
